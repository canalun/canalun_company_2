import {
  type Bar,
  getBarCenterPosition,
} from "@/utils/brick-block-anywhere-demo-funcs/bar.ts";
...
export function updateDirectionByCollisionWithWall(
  ...
  if (globalThis.innerWidth <= maxXOfCollisionPointsOnBall) {
    return { x: -1, y: currentBallDirection.y };
  }
  if (minXOfCollisionPointsOnBall <= 0) {
    return { x: 1, y: currentBallDirection.y };
  }
  if (globalThis.innerHeight <= maxYOfCollisionPointsOnBall) {
...
export function updateDirectionByCollisionWithBar(
  ...
  const mostBottomPointsOnBall = collisionPointsOnBall.find((v) =>
    v.y === minY
  );
